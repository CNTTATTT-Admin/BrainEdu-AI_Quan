class UserProfileBuilder:
    ...
    def build(features):
        ...
        completed_courses = features.get(
            "completed_courses",
            []
        )

        total_learning_time = features.get(
            "total_learning_time",
            0
        )

        lesson_completed_count = features.get(
            "lesson_completed_count",
            0
        )

        content_preferences = features.get(
            "content_preferences",
            {}
        )
        ...
        project_score = content_preferences.get(
            "project",
            0
        )

        video_score = content_preferences.get(
            "video",
            0
        )

        article_score = content_preferences.get(
            "article",
            0
        )

        if project_score >= max(
            video_score,
            article_score
        ):
